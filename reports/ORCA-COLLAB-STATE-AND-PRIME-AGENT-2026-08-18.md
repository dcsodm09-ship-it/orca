...
### Round 46：Claude opus/max 给出真正的 GO——0 P0/P1/P2，只有 6 条防御纵深性质的 P3（等待 Codex 一路确认真正双 GO）

**Claude opus/max：GO，0 P0、0 P1、0 P2、6 P3**——这是迄今为止最
彻底的一轮复核，方法本身值得记录，不只是"没找到东西"：

- **独立从零重新推导了 round 45 的修复**：没有直接采信报告，自己
  拿真实 Node v24.19.0 重建对照——33 个手选 `HOME` 用例 + 3961 个
  随机模糊测试用例（`.`/`..`/空/重复分隔符/unicode/空格片段），
  667 处偏差**全部**能用"开头双斜杠"这一个已知原因解释、零个
  解释不了的偏差；用 `os.stat().st_ino` 现场确认 `//x`、`///x`、
  `/x` 在这台 Darwin 文件系统上是同一个 inode；确认了机制本身对得上
  Node 真实源码路径（`Module._initPaths` 里 `path.resolve(homeDir,
  ...)` 读的是 `safeGetenv('HOME')`，正是这个安装器传给唯一那个
  子进程的、经过清空的同一份环境）。走完整真实链路复现：pre-fix
  基线 `76b3a24f36` guard 退出码 0、Node 真的被启动、攻击者目录真的
  进了 `globalPaths`；修复后 `f152c108d9` guard 退出码 78、拒绝、
  点名具体路径。
- **专门扫了一遍"同样形状的问题是不是还有别处"**：清点了全部从
  攻击者可控输入拼路径、再走文件系统存在性检查的位置，找到两处
  没被归一化、且确实是环境变量来源的——`prime_agent_search_
  directories()`/`prime_agent_command_candidates()`（用 8 组场景
  实测，方向和 round 44 那个漏洞相反：这里"不归一化"最多只会**多**
  出候选项、任何多出来的候选项本身就会被拒绝，不存在漏判方向）；
  生成的 shell wrapper 里那句 `[ -e "$PWD/.prime/agent/
  settings.json" ]`（实测确认非交互 `/bin/sh` 会用真实 cwd 校验
  并替换掉过期的 `$PWD`，静态绕过不成立，只留一个概率极低的
  TOCTOU 残留，见下面 P3）。
- **专门挑了 5 个最近几轮没被重点看过的、完全不同类的方向**，都是
  真跑代码验证、不是纸面推理：(i) shell/dyld 层面的环境变量投毒——
  构造真实执行链路、注入 47 个已知危险变量，**零个**真正到达 node
  子进程，确认 `BASH_ENV`/`ENV` 不会被非交互 `/bin/sh` 读取，
  `DYLD_INSERT_LIBRARIES` 对 `/usr/bin/python3`/`/bin/sh`（SIP
  限制）完全无效——但对 Homebrew python3**确实**有效，说明硬编码
  `/usr/bin/python3` 这个选择本身承担了比现有注释里"`-B -I` 隔离"
  更多的安全含义、目前没写进文档；(ii) receipt/journal 持久化与
  恢复路径——完整读了隔离/恢复/finalize 全部逻辑，回滚路径判断
  健全，恢复目的地经过校验、manifest 里的 `items` 是四个字面量的
  白名单、没有穿越空间，找到一处防御纵深不对称（见 P3）；
  (iii) uninstall/enable 和命令分发——`_uninstall_locked` 只做
  禁用、`remove_exact_symlink` 严格校验、`_enable_locked` 会先跑
  完整 `verify()` 才继续，`argparse` 走的是封闭 `choices=` 元组、
  没有 shell/eval 注入面，`verify()` 里五个钉住摘要全部在任何
  node/npm 执行**之前**检查完毕；(iv) 自身身份/位置发现——整个
  安装器**没有任何地方**用 `__file__`/`sys.executable`/
  `os.getcwd()`，全部路径都是模块级常量，根本不存在可伪造的攻击面；
  (v) 真实路径里带空格（`Extreme SSD`）会不会干扰 `ps` 解析——
  构造了一个 argv 精确模仿真实托管进程、在带空格路径下运行，确认
  匹配逻辑是对整个参数字符串做子串搜索、不是对被空格截断的字段，
  依然正确识别，fail-safe。
- **4 条新回归测试全部核实为真**：对着 round-45 基线 `76b3a24f36`
  单独重跑，两条核心测试确实失败（`guard rc=0`、候选表里还留着
  未归一化字符串），修复后通过；round 45 提交信息里"只有两条核心
  测试真的会 pre-fix 失败"这句话准确、没有夸大。
- **6 条 P3**（均判定无实际可利用性，纯防御纵深/文档建议）：
  `resume_incomplete_quarantine()` 缺少 `quarantine_partial_
  release()` 已有的激活态守卫，但通过 API 无法真正触发（需要
  攻击者已经拥有 `TOOL_ROOT` 写权限、且两个真正调用点在能创建
  receipt 之前就已经调用过 resume）；`$PWD` 那处项目设置门禁在
  静态场景下站得住，只剩一个需要"先满足一整条不太现实的前提链 +
  再赢一次亚毫秒级竞速"的残留；`SHELLOPTS=xtrace` 能在 wrapper
  shell 里执行代码，但对照测试证明同样的攻击者靠简单地在 `PATH`
  上遮蔽 `prime-agent` 就能拿到完全相同的执行能力、wrapper 根本
  不会跑，和 round 40 那种"注入进已经被验证过的托管进程、guard 还
  报告成功"性质完全不同；`sha256_file()` 现在是死代码（所有调用点
  都迁移到了 `sha256_file_verified()`），文档字符串警告过不要用、
  但没有强制手段，留下"未来某个调用点悄悄重新引入 round 29 那个
  漏洞"的隐患；`atomic_write()` 里 `os.replace` 之后那次
  `os.chmod` 是多余的（fd 早就 `fchmod` 过了）、只是收紧、影响
  可忽略；`/usr/bin/python3` 的 SIP 限制状态是真正承担安全职责的、
  但现有注释只提了 `-I`/`-P` 这一个理由，值得补充记录。

opus/max 的收尾判断："46 轮走到这里，'没找到东西'只有在搜索本身
是结构化的时候才有意义。我这一轮是结构化的：把 round 44 那类
漏洞形状的完整站点清单列出来，证明剩下两处都是失败关闭；把 round
45 的修复从零对着真实 Node 重新推导了一遍（3994 个用例）、走完整
链路在修复前后各复现了一次；另外挑了 5 个完全在环境变量/模块解析
这条线索之外的方向，全部用真跑代码而不是读代码去攻击。环境变量/
模块解析这条线索确实关死了，我找到的东西没有一条超出防御纵深的
级别。"

**Codex round 46 一路的最终结局**：真的干了约 19 分钟活（心跳确认
过真实进度，不是一开始就卡住），之后撞上同一个 OpenAI 自己的
"Trusted Access for Cyber"内容策略墙、彻底停住。已停止、清理、
如实记录。**这是这整条 prime-agent 复核线索里第 4 次撞上这同一个
基础设施问题**（round 35、round 42 那次旧提交复核、这次 round
46）——已经不太像偶发，值得向用户请示怎么处理，而不是继续无限
重试同一种做法。**按用户自己的规则，单路 GO（哪怕这一路质量很高）
...
### Round 48 Claude opus/max 一路结果：**GO，0 P0，0 P1，2 P2，6 P3**——但挖出一个值得认真对待的具体问题：CLI 侧残留窗口比文档写的大 15-50 倍，而且"CLI 侧完全做不到描述符绑定"这个结论本身是错的

这一路复核方法论很扎实：先独立核对了提交沿革（任务给的 HEAD 是
`8ec03e28fd`，实际当时 HEAD 已经推进到 `19dc0a1393`，复核过程中又推进到
`0adbbae159`——每次都重新核对了 `install_prime_agent.py` 字节级不变，用的
是同一份 SHA-256）；用 AST 解析确认了 `validate_exec_target()` 等三个函数
其实不是顶层函数，而是嵌在 `managed_launch_guard_script()` 里的 f-string
模板，真正物化出来跑的是安装时生成的独立脚本（62875 字节），两个解释器都
编译通过；描述符生命周期逐条核对了全部 7 条返回路径，用 `/dev/fd` 现场
测量确认 0 处泄漏；子进程不会继承新打开的描述符（PEP 446 默认不可继承 +
`close_fds=True`）；用"剥掉两处 reassertion 调用、保持其余全部不变"的
变异测试直接证明了这个修复确实是 load-bearing 的——剥掉后同样的注入替换
就会让 NODE 真的跑起被替换的 CLI 内容。

**但独立重新做了一遍 `/dev/fd` 可行性验证后，发现两个具体问题**：

- **P2-1**：round 47 文档把残留窗口描述成"最后一次 `lstat()` 到
  `subprocess.run()` 内部再次 exec 之间"，对 NODE 侧准确（约 1-4ms），
  但对 **CLI 侧严重低估**——因为 NODE 是先被 exec、CLI 是等 NODE 自己
  完整启动之后才会被打开的，这一路用真实 Node v24.19.0 实测出这段真实
  窗口是 **41-229ms，中位数 55ms**，比文档暗示的窗口大了一到两个数量级。
  更关键的是，这一路直接对着**真实生成的 guard + 真实钉定的 Node**，
  在两次 reassertion 都已经通过之后的 0/1/5/10/20/30 毫秒六个延迟点分别
  注入替换，**全部六次**都让真实 Node 真的执行了被替换的攻击者 CLI
  内容。这一路明确指出这不是新引入的回归（round 47 之前的窗口更大，
  包含整个 ~100MB Node 二进制哈希计算加祖先目录遍历，round 47 确实把
  窗口收窄了），但"文档描述的窗口大小"本身和"真实测出来的窗口大小"
  对不上，判定为 P2（准确性问题，不是新缺陷）。
- **P2-2 / 更值得注意的一点**：这一路自己独立写测试重新核实了 round 47
  "两边都做不到描述符绑定"这个结论，NODE 侧确认属实（而且真实原因比
  round 47 文档写的更绝对——Darwin 拒绝通过 `/dev/fd` exec 是**不分打开
  模式的**，不是 `O_RDONLY`/`O_EXEC` 互斥这么简单，round 47 自己保留的
  `smoke_dev_fd_exec.py` 脚本里其实已经写对了这一点，只是 `install_
  prime_agent.py` 正文的文档字符串归因写错了）。**但 CLI 侧的"做不到"
  结论被证明是过度声称**：这一路用一个自定义的 `Module._compile`
  引导方式（从描述符读取内容，同时保留 `__dirname`/`__filename`/
  `module.paths` 不被破坏）证明 CLI 确实可以做到真正的描述符绑定——
  替换路径指向攻击者代码后，引导方式跑出的是真实 CLI 内容
  （`sibling.js` 兄弟文件导入也正常），而不是被替换的攻击者代码；代价
  是会让 `require.main === module` 从 true 变 false、`process.argv`
  行为也会变化，这是需要额外处理的兼容性问题，但**不是"做不到"**。
  巧的是，真正可以被描述符绑定关掉的那一半（CLI）恰好就是残留窗口
  更大、更容易被真实利用的那一半——round 47 因为误判"两边都做不到"而
  放弃了本来可以实际堵住的、影响更大的那个口子。

6 个 P3：`validate_receipt_identity()` 对未在 32 键白名单里的 9 个键不做
约束（仅同 UID 静态攻击者才能利用）；新增字符串存在性断言没有约束调用
顺序（重构把 reassertion 挪到更早的位置测试也会通过）；
`test_pre_fix_...` 测试本身是叙事性复现、不是真正的回归测试（测试自己
的文档字符串已经如实说明）；`os.close()` 自身抛异常的理论双关闭窗口（有
`finally` 兜底，单线程流程下无 fd 复用窗口）；`validate_exec_target` 里
`os.path.commonpath` 可能抛 `ValueError`、不在 `main()` 的
`except (OSError, subprocess.SubprocessError)` 覆盖范围内（用当前写死的
绝对路径常量不可达）；NODE 侧文档字符串对不可行原因的归因写错（真实原因
是 Darwin 无条件拒绝、不是打开模式互斥）。

其余"这轮要格外广"的部分：真跑了 124633 个非豁免用例模糊测试参数扫描层
（0 处违规）；生成的 shell 入口点在 `sh`/`bash`/`dash` 下语法干净，
`$PWD` 门禁实测不可被伪造，针对性复现了 round-10 那种 `help <arg>`
绕过手法并确认对着真实上游 0.7.2 bundle 已经堵死；tar 解包、隔离恢复
生命周期都重新读通确认加固到位。

187/187 测试 4 次全过（双解释器各两次），`py_compile` 6/6 干净，2 次真实
`sandbox_e2e.py` 全 `ok:true`、`production_lock_sha256`/`license_sha256`
和钉定值精确一致。两条新回归测试逐条核实为真（其中一条用变异测试证明确
实 load-bearing，另一条如实标注自己是叙事性复现而非真正回归测试）。

**这一路给出的最终结论是 GO（0 P0/P1），但 P2-1/P2-2 揭示的问题本身很
具体、很有说服力，而且给出了一个此前被误判为不可行、实际可以关掉更大
风险敞口的修复方向。** 按用户规则，0 P0/P1 已经技术上满足"可以完成"的
字面标准；但鉴于这个发现的具体程度（真实攻击复现、真实测量出的窗口、
真实证明可行的关闭路径），在宣布这个候选完成前，先等 Codex sol/max
...
完全一致，和本轮改动无关。这是一次真实的上游发布事件，不是本机架构
问题。**这正是这条复核线索从第 1 轮起就一直独立标注的"上游锁定哈希
漂移"问题**（2026-08-14 采集的证据和当时/后续 registry 实际状态不
一致），现在确认它依然存在，这意味着即便安全复核彻底收敛，
`GENERATED_LOCK_SHA256` 也需要人工对照最新上游证据重新采集、核对、
钉一次，这本身是一次独立于代码安全复核的信任判断，本轮仍未擅自处理。

**更正（round 50 opus/max 复核指出）**：round 49 修复 agent 当时给出的
具体归因——"`@smithy/core@3.33.3` 在真实 npm registry 上已经不存在了，
当前最新是 `3.33.2`"——**这句话本身写反了**，round 50 的 opus/max 一路
指出后，我直接查了真实 registry 独立核实：`dist-tags.latest` 就是
`3.33.3`，发布时间 `2026-08-20T16:03:38Z`；`3.33.2` 才是更早（发布于
2026-08-15）的旧版本。真实情况是**反过来的**：`3.33.3` 是刚发布不久的
新版本，不是被撤下的版本。"上游锁定哈希漂移仍然存在、需要人工重新
钉哈希"这个结论本身没错（隔离 worktree 对照的核心证据依然成立），但
round 49 那句解释具体机制的话是错的，之前也原样转述给了用户，这里
一并更正，不让错误的具体归因留在记录里误导后续判断。
...
### Round 50 Claude opus/max 结果：**GO，0 P0/P1，2 P2，5 P3**——但其中一个 P2 是本轮修复自己引入的新回归（FIFO 拒绝服务），另一个是这次探索顺带挖出的、和本轮无关的既有真实缺陷

方法论上这一路做得非常扎实：从真实 `edf8b40860` 的 git blob 里用 AST 抽出
修复前的函数本体去复现（不是自己手写一个近似版本）；确认了修复后能挡住
立即替换、预先打开、硬链接、内容相同的新文件、符号链接指向目录、悬空
符号链接等至少 6 种不同的替换形状；专门跑了一个**真实、零 mock 的 6 秒
多进程竞态**——19794 次真实 `atomic_write()` 调用、其中 19710 次被真的
换成了符号链接，结果是全部安全失败、victim 文件的权限一次都没被误改。
还用"直接读取硬件级 fd 计数"的方式核实了新加的 `verified_descriptor`
在四类路径（ELOOP/身份不符/内容不符/正常路径）下各跑 120 次都没有描述符
泄漏。

**P2-1（本轮修复自己引入的新回归，最值得关注）**：新加的
`os.open(path, O_RDONLY | O_NOFOLLOW)` 如果在 `path` 这个位置被替换成一个
FIFO（命名管道），会**无限期阻塞**——复现确认：修复前同样的操作
0.0007 秒就返回，修复后会挂起 5 秒以上，直到有人往那个 FIFO 里写数据才
会解除阻塞。这个阻塞发生在还持有 `exclusive_lifecycle_lock` 的时候，会
连带卡住每一个生命周期操作（包括 `recover` 本身）。人工交互式使用能用
Ctrl-C 脱身（已验证），但非交互式/自动化运行会一直挂到超时——是一个真实
的拒绝服务面。**修复方式很便宜**：照抄同文件里 `read_private_file()`
自己已经在用的写法——`open()` 之前先 `lstat` 一次、判断 `S_ISREG`，不是
常规文件直接拒绝，根本不去 `open()` 它。

**P2-2（既有缺陷，与本轮改动无关，是这一路"这轮要格外广"的要求下顺带
挖出来的）**：一份被同 UID 攻击者投毒的 recovery manifest（比如塞进一个
孤立代理项 `\ud800`）会让 `canonical_json()` 抛出未捕获的
`UnicodeEncodeError`（`strict_json()` 对 `NaN`/`Infinity`/孤立代理项这些
非 RFC 严格 JSON 值本来就来者不拒，200000 层嵌套还会让 `RecursionError`
直接逃逸，因为 `main()` 只捕获 `PrimeInstallError`）——链式复现到 CLI
层级：`install`/`recover` 两条路径都会因为同一个损坏字节序列**永久卡死
在空 stdout + 裸 traceback**，而不是这个工具自己一直坚持的
`{"ok": false, "error": ...}` 契约。这两条恰好是操作员遇到问题时最先会
用来自救的命令，被同一处缺陷同时堵死。

5 个 P3：新加的内容校验和最终 `fchmod` 之间仍然存在一个真实但**结构上
无法彻底关闭**的 TOCTOU（复现确认：在校验读完之后、`fchmod` 之前那一刻
原地改写同一个 inode，函数仍然报告成功——但这一路的判断是这不是编码
缺陷，是"针对一个持续拥有写权限的同 UID 攻击者做检查后动作"这类模式
本身固有的极限，攻击者随时可以在函数返回后的下一纳秒再写一次；docstring
里"重读内容排除这种情况"这句话确实说得有点满，应该改成更克制的表述）；
没有像 60 行之外的 `read_private_file()` 那样在读完后再 `fstat` 一次比对
`(st_dev, st_ino, st_size, st_mtime_ns)`，所以"校验读完之后又被追加内容"
这种情况检测不出来；`quarantine_partial_release()` 对"验证 open() 之后才
发生"的这类更晚时机竞态，返回值仍可能是 `state=quarantined`（但下游
消费者会重新校验、真正意义上的"耐久性"没有被突破）；`strict_json`/
`canonical_json` 允许非 RFC JSON 值并把它们原样写回这个安装器自己的
证据文件；以及**round 49 提交信息里对 `sandbox_e2e.py` 那次上游漂移的
具体归因写反了**（已在上面单独更正，见本节前段）。

对本轮**特意跳过未修的两处**（`safe_extract_main_asset()`、
`extract_node_toolchain()`）也独立复核确认：两处后面都紧跟着
`assert_tree_has_no_symlinks()` 遍历整棵树、任何符号链接都会让安装直接
中止——也就是说"返回成功、但路径其实是攻击者符号链接"这个 P1 意义上的
真正后果在这两处根本不会发生，只是 chmod 的副作用会落空、然后操作本身
就会失败退出，可利用程度明显低于本轮修的那处。**判断这次的取舍范围
是合理的。**

191/191 测试 4 次全过（双解释器各两次），`py_compile` 干净；`sandbox_e2e.py`
2 次真实运行都因为上游锁定哈希漂移而 `exit 2`（已用隔离 worktree 独立
复核过这个漂移和本轮改动无关）；4 条新回归测试逐条核实为真，还额外做了
"把修复后的 `atomic_write` 换回真实修复前版本、其余不变"这种置换测试，
确认 3 条"修复后"测试确实全部会失败——不是碰巧通过。

**这一路最终结论是 GO**，但 P2-1 是本轮修复自己引入的、有明确复现和明确
低成本修法的真实回归，不应该被无视；P2-2 虽然和本轮无关，但严重程度
（同时堵死 install 和 recover 两条主要自救路径）值得尽快单独排期。

### Round 50 Codex 一路中途遇到真正硬阻断：账号 ChatGPT/Codex 用量额度用完（提示到 2026-09-19 才重置）——不是可恢复问题，用户亲自换了账号后恢复

Codex 那一路（`task_354bdc116876`）在给出前面提到的那条 escalation
之后就再没有心跳——查真实终端发现它先撞上一个此前没见过的新提示
（"Switch to gpt-5.6-luna for lower credit usage?"，选了"3. 保持当前
模型（以后不再提示）"以维持 `terra/high`），随后立刻撞上真正的硬阻断：

> You've hit your usage limit. Upgrade to Plus to continue using Codex...
> or try again at Sep 19th, 2026 11:07 PM.

这和此前遇到的所有阻断（hooks 信任竞态、Trusted Access 内容策略墙）性质
完全不同——**不是几分钟内可以靠换模型/换终端恢复的问题，是账号级的月度
额度耗尽，官方给出的重置时间在一个月之后**。没有再自己尝试绕过或猜测
账号切换方式，直接向用户请示怎么处理。用户回复"已经换好codex了"（自己
交互式处理了账号切换，不是我操作的）。核实 Orca 运行时确实已经重启过一次
（`runtimeId` 变了），原终端已被关闭（`exitCause: operator_close`）。

换账号后重新派发：`gpt-5.6-sol/max` 这次不再报"账号不支持这个模型"，
派发过程照例撞了一次已知的 `hooks.json` 信任竞态，用标准手法恢复后确认
终端真实在跑（能看到真实工具调用输出）。但这次重新挂载到 Orca 追踪
连续两次报 `agent_prompt_stalled`——不是新问题，是因为终端当时正忙着干活，
Orca 自己的就绪检测在这种"终端正在处理中"的时间点去探测会判定失败；
终端本身没有任何异常。没有为了强行挂载追踪去打断它正在做的真实工作，
...
### Round 53 Claude opus/max 结果：对这次重新钉哈希本身是 **GO（0 P0/P1，4 P2，5 P3）**，但顺带挖出了三件比"这个哈希对不对"本身更重要的事

这一路复核方法论极其扎实：自己重新写了一遍归一化逻辑（不是照抄安装器
的代码去跑，是独立实现同一个算法），再单独跑了一遍真实隔离 npm 重放，
两种方式各自独立复现出同一个哈希——这是**第三、第四次**独立复现，全部
落在同一个值上。还去查了 3 个变化版本的 attestation bundle（不只是查
元数据），把 Sigstore 证书解码确认签发者、有效期、CI 运行 ID，甚至把
6 个 tarball 全部下载、算出 SHA-512，逐一比对和 attestation 里
`subject[0].digest.sha512` 完全一致——证明"npm 真正会装的字节"和"被
签名 provenance 覆盖的字节"是同一份。还真的去读了变化的 3 个包的
CHANGELOG 和源码 diff，确认改动内容本身是良性的（`for-in` 循环加
`hasOwnProperty` 防护、指数记数法数值修复），没有新增依赖、没有安装
脚本。**这次重新钉的哈希本身：确认正确，GO。**

但这一路做的"这轮要格外广"的部分，挖出了三件更大的事：

1. **（P2-3，最重要）"和上游钉的证据比对"这个方法论本身有一个结构性
   盲区，会漏掉最需要关心的那类漂移**：现有方法只查"相对上一次自己钉
   的哈希，这次变了哪几行"，从来不查"相对上游 v0.7.2 自己测试过的那份
   完整 `package-lock.json`，现在实际装的这份闭包已经漂到哪去了"。这
   一路真的做了这个对比：196 行里约 35 行的解析版本和上游自己那份锁
   不一样，**其中一个不是小版本升级，是主版本跳跃**——`zod` 从
   `3.25.76` 跳到了 `4.4.3`（由 `@anthropic-ai/sdk`/`openai`/
   `@mistralai/mistralai` 等几个包共同声明的 `^3.25.0 || ^4.0.0` 范围
   触发，上游当时解析到的是 3.x）。这意味着 README 里"所有安全复核
   轮次都是针对 v0.7.2 精确打包的 JS 验证过的"这句话，只对被本地
   patch 的那 4 个包成立——它们运行所依赖的这一整棵 196 个包的依赖树，
   其实**不是**上游真正测试过的那棵树，而且每隔几天就会再漂一次。
2. **（P2-4）量化出了"为什么这条线索反复撞上过期哈希"的根本原因**：
   仅 `@smithy/core` 一个包，从 6 月 1 日至今就发布了 24 个版本，中位
   发布间隔 2.13 天。这一路给出一个具体数字——`GENERATED_LOCK_SHA256`
   的"经验半衰期大约是两天"，预计到 2026-08-23 左右就会再次过期。
   指出了真正的风险：fail-closed 方向是对的，但如果每次过期都靠"人工
   在时间压力下重新核实、再放行"来解决，这正是"审查逐渐变成走过场"
   最容易发生的模式。给出的结构性修法：既然安装器本来就已经下载、
   摘要钉定了上游自己的完整 463 行 `package-lock.json`，应该直接从
   那份**已经被信任**的上游闭包播种生成锁，而不是每次都做一次不受
   约束的、会自由浮动的全新解析——这样能同时关掉上面那条 P2-3。
3. **（本轮范围之外，但主动去查了）`extract-zip@2.0.1` 是一个真实、
   目前无补丁的高危漏洞（GHSA-jmr9-qjv8-65gv，CVSS 8.1，未校验的
   符号链接路径穿越），且这整条流水线完全看不见任何 npm 安全公告**：
   查证是 `managed_npm_environment()` 显式设置了
   `npm_config_audit=false`，导致 `npm install` 从来不会检查/上报任何
   已知漏洞公告。这一路自己直接查了 npm 的批量 advisory 接口，对全部
   192 个钉定版本逐一核对，**只有一个命中**：`extract-zip@2.0.1`——
   这是 prime-agent 的直接生产依赖，精确钉定自上游自己的锁（不参与
   本轮的浮动解析，所以这次重新钉哈希不会改变它），2.0.1 从
   2020-06-10 至今是这个包发布过的最新版本，**从未有过修复版本**。
   这本身不是本轮改动引入的新问题，是早就存在的上游问题，但恰好是
   这类"上游信任复核节奏"本该但从来没有捕捉到的那类东西。

5 个 P3（较轻）：README 里"GitHub release 资产一旦发布就不可变"这句
话事实上不准确（删除重传技术上是允许的，只是这次真的没发生，且
SHA-256 钉定本身会 fail-closed 兜底，不构成真实风险）；"source lock
是 v0.7.2 自己的直接依赖"这个旧表述其实指的是完整 463 行闭包，新段落
已经写对了；"3 of 196"严格说是 196 行里对应 192 个去重后的
`name@version`；"两次完全独立的重放"这句话本身没错，但因为都在同一台
机器、几分钟内跑完，独立性没有想象中那么强——真正扛住结论的是逐行
provenance 核查，不是重放次数本身。

测试套件确认了一个值得记的事实：**206/206 测试对这次改动其实是"不
沾边"的**——29 处测试引用都是 `monkeypatch` 掉 `GENERATED_LOCK_SHA256`
本身，没有一处硬编码真实哈希值，所以"测试全过"对这次具体改动没有
证明力；真正扛住结论的是不带 `--expected-lock-sha256` 跑的真实
`sandbox_e2e.py`（这一路自己也跑了两次，都是真实 `exit 0`、
`ok:true`，哈希吻合）。

**综合结论**：这次重新钉的哈希本身没问题，opus/max 这一路对
`bafce01bf5` 给 GO；但挖出的三件事——上游漂移方法论的结构性盲区、
两天半衰期带来的"审查会不会被时间压力磨成走过场"风险、以及一个从未
被这条流水线看到过的真实高危上游漏洞——分量都不小，值得等 Codex 那
一路结果出来后一起综合，向用户请示是不是要单独开一轮处理（不是这次
哈希更新本身的阻断项，是这套上游信任机制往后怎么做得更扎实的问题）。
