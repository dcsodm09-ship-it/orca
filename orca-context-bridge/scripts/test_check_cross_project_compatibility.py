...
class AtomicWriteWithinBaseDirTocTouTests(unittest.TestCase):
    ...
    BASENAME -- it does nothing about base_dir itself being swapped for a
    symlink to an outside decoy in the gap between the caller's earlier
    check and this call. Repro follows this repo's own established
    dir-fd-swap methodology (see promote_capability.py's
    ...
    the real function is about to open base_dir, then confirm the open
    refuses outright (O_NOFOLLOW on base_dir's own now-symlinked name)
    instead of silently writing through it into the decoy."""

    def setUp(self) -> None:
        self.tmp = Path(tempfile.mkdtemp(prefix="ccc-atomic-write-dirfd-"))
    ...
    def test_base_dir_symlink_swap_before_dir_fd_open_fails_closed(self) -> None:
        ...
        real_opener = ccc._open_atomic_write_dir_fd

        def swap_then_open(base_dir):
            os.rename(str(self.real_dir), str(self.moved_aside))
            self.real_dir.symlink_to(self.outside_decoy, target_is_directory=True)
            return real_opener(base_dir)

        with mock.patch.object(ccc, "_open_atomic_write_dir_fd", side_effect=swap_then_open):
            with self.assertRaises(OSError):
                ccc.atomic_write_within(self.real_dir, final_path, b'{"x": 1}')

        # The exact exploit this closes: the outside decoy must never
        # receive the write.
        self.assertEqual(list(self.outside_decoy.iterdir()), [])
        # Nothing landed in the real (moved-aside) directory either --
        # the open must fail BEFORE any tmp file is ever created.
