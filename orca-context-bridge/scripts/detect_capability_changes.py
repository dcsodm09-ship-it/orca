...
def _write_all_bytes(fd: int, payload: bytes) -> None:
    ...
    same project's own round-6-fix final-gate review empirically
    demonstrated a genuine short write on this exact machine via
    RLIMIT_FSIZE: os.write() returned fewer bytes than requested with no
    exception raised). The caller of this function relies on the tmp file
    it writes being either the COMPLETE intended payload or absent --
    silently accepting a short write here would let a truncated,
    invalid-JSON tmp file get os.replace()'d onto the live
    capability-content-hashes.json / capability-changes.json output while
    the caller still reports success. Loop until every byte is written; a
    ...
    with a silently truncated result. Copied, not imported, from
    wiki_edit_guard.py's own `_write_all_bytes()` -- see this module's
    docstring on why the write-guard functions here are an independent
...
def atomic_write_within(output_dir: Path, final_path: Path, payload: bytes) -> None:
    ...
    # Opened on final_path.parent, not the output_dir parameter: for every
