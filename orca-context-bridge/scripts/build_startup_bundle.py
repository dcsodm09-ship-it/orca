...
# Paths (relative to knowledge_root) the authority git-freshness check is
# scoped to, so unrelated churn elsewhere in a large, actively-developed
# knowledge_root repo does not perpetually invalidate a signed manifest.
# "wiki" holds shared-source content not already covered by an individual
# shared_source_sha256s pin (e.g. reference docs alongside the pinned
# capabilities/wiki JSON files). "orca-context-bridge" is knowledge_root's
# own copy of the bridge sources, kept for audit/review history; it is a
# separate, not-necessarily-in-sync copy of whatever is actually deployed
# and executing (see --expected-generator-sha256 in the hook config, which
# only pins startup_context.py, not this file or its other siblings) — this
# check does not attest to the currently-running code.
AUTHORITY_TRACKED_PATHS = ("wiki", "orca-context-bridge")
...
def summarize_git(
    ...
    status_argv = ["git", "status", "--porcelain=v1", "-z"]
    if untracked_all:
        # Recurse into fully-untracked directories instead of collapsing each
        # to a single line, so the fingerprint changes when a file is added
        # to or removed from them, not just when the directory first appears.
        # NOTE: git status reports path + status letter only, never content —
        # editing an existing file in place while its tracked/untracked
        # status letter is unchanged does NOT change this fingerprint. That
        # is fine for paths whose actual content is separately pinned by an
        # exact sha256 (see shared_source_sha256s / pack.sha256 below, which
        # this check does not replace); it is a real limit for anything
        # under the scoped paths that is *not* individually content-pinned.
        status_argv.append("--untracked-files=all")
    status_argv.append("--")
    if paths:
        status_argv.extend(paths)
    else:
        # Startup receipts and launch snapshots are generated under
        # .orca/context. Exclude that private runtime directory so one
        # provider's delivery cannot change the content-addressed bundle
        # observed by a concurrent provider.
        status_argv.extend([".", ":(exclude).orca/context", ":(exclude).orca/context/**"])
    status_raw = run_text(status_argv, project)
...
def verify_reviewed_pack(
    ...
    # Scoped to AUTHORITY_TRACKED_PATHS with deep untracked-file recursion:
    # knowledge_root is a large, continuously active multi-project workspace,
    # so an unscoped whole-repo status hash drifts on every unrelated WIP
    # file and makes freshness unsatisfiable in steady state, while a
    # scoped-but-shallow hash would collapse an entire untracked directory
    # to one line and miss files being added to or removed from it. This is
    # a path-set fingerprint, not a content fingerprint (see summarize_git's
    # untracked_all note above) — the content that's actually delivered as
    # reviewed context is separately pinned above via shared_source_sha256s
    # and pack.sha256, which this check does not substitute for.
    authority_git_state = summarize_git(
        knowledge_root, paths=AUTHORITY_TRACKED_PATHS, untracked_all=True
    )
