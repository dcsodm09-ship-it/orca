...
export function migrateLegacyContractStorage(this: OrchestrationDb): void {
  ...
  // Why this runs here, between creating the table above and the 2 more below (see its own
  // doc comment for the full history): it must see the table already created, and its own
  // 2 unique indexes must exist before anything else assumes duplicates are impossible.
  this.dedupeLegacyCompatibilityPrincipals()
